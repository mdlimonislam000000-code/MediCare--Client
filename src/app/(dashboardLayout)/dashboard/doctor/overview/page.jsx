...
const DoctorOverview = () => {
  ...
  const completedAppointments = bookings.filter((booking) => {
    ...
    return prescriptions.some(
      (prescription) =>
        prescription.bookingId === booking._id && booking.doctorUserId === doctorId
    );
  }).length;
  ...
  const clinicianScore = totalFeedbacks > 0
    ? (reviews.reduce((acc, curr) => acc + Number(curr.rating || 0), 0) / totalFeedbacks).toFixed(1)
    : "0.0";
