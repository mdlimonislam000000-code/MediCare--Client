...
const DoctorSchedulesPage = () => {
  ...
  const fetchSchedule = (email, id) => {
    ...
    fetch(
      `${process.env.NEXT_PUBLIC_SERVER_URL}/api/doctor/schedule?${queryParam}`,
    )
      .then((res) => res.json())
      .then((data) => {
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">
              Configure your weekly working days and available consultation
              hours.
            </p>
          </div>
        ...
        {fetching ? (
          <div className="min-h-[30vh] flex items-center justify-center">
            <h2 className="text-lg font-semibold animate-pulse text-indigo-600 dark:text-indigo-400">
              Loading Schedule...
            </h2>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white dark:bg-zinc-900 p-6 rounded-2xl shadow-xl border border-zinc-200 dark:border-zinc-800 space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span className="text-xs bg-zinc-100 dark:bg-zinc-800 px-3 py-1 rounded-full text-zinc-600 dark:text-zinc-400 font-medium">
                  Configure Days
                </span>
              </div>
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                {workingDays.length === 0 ? (
                  ...
                  workingDays.map((day, idx) => (
                    <span
                      key={idx}
                      className="badge badge-lg bg-emerald-100 text-emerald-800 dark:bg-emerald-950/60 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800 gap-2 p-3 font-medium"
                    >
                      {day}
                      <button
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                {timeSlots.length === 0 ? (
                  ...
                  timeSlots.map((time, idx) => (
                    <span
                      key={idx}
                      className="badge badge-lg bg-cyan-100 text-cyan-800 dark:bg-cyan-950/60 dark:text-cyan-300 border border-cyan-200 dark:border-cyan-800 gap-2 p-3 font-medium"
                    >
                      {time}
                      <button
