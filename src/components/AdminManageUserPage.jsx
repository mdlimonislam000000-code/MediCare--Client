...
const AdminManageUserPage = () => {
  ...
  const fetchUsers = () => {
    setLoading(true);
    fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/users?role=${activeTab}`)
      .then((res) => res.json())
      .then((data) => {
        setUsers(data);
        setLoading(false);
  ...
  const handleSuspend = (id) => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/users/suspend/${id}`, {
      method: "PATCH",
      headers: {
  ...
  const handleUnsuspend = (id) => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/users/unsuspend/${id}`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
  ...
  const handleDelete = (id) => {
    ...
    if (
      window.confirm(
        `Are you sure you want to delete this ${activeTab} account?`,
      )
    ) {
      fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/users/${id}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-slate-600 dark:text-gray-400 mt-1">
              Manage doctor and patient accounts, suspend or delete them as
              needed.
            </p>
          </div>
